...
> 값**입니다(§5 참조). 각 런의 `resources.jsonl`에서 `gpu_used_gib`가 전 구간 7.76 GiB
...
것과 정반대(§3)이며, 동적 배칭을 붙이면 GPU당 로봇 수용량이 2대 → 6대가 되지만,
그보다 더 잘게 쪼개는 스케줄링(denoise 링)은 같은 이유로 역효과입니다(§4).**
...
### 권장 순서 (예측) — §4에서 실측으로 뒤집힙니다

여기까지가 비용 모델에서 나온 예측입니다:

| 단계 | 처리량 | 지연 안정성 | 비용 |
| --- | --- | --- | --- |
| 1. 동적 배칭 | **2 → 6대** | — | 서빙 계층 2곳 |
| 2. continuous batching (denoise 링) | 변화 없음 | 대기 단위 216 → 5.3 ms | 스케줄러, 그래프 경계 추가 없음 |
| 3. 레이어 축 chunked prefill | 변화 없음 (−4 ms) | prefill stall 163 → 27 ms | 레이어 구간 API |

> ⚠️ **2·3단계는 실측에서 성립하지 않았습니다.** §4에서 링을 실제로 구현해 재보니 두
> 부하 구간 모두에서 가장 나빴습니다. 위 예측이 놓친 것은 "그래프 경계 추가 없음"이라는
> 전제입니다 — 경계 수는 그대로여도 **요청당 실행 횟수가 1회에서 11회로 늘어나고**,
> §2의 호스트 바운드 박스에서는 그것이 그대로 비용입니다. 1단계만 유효합니다.

## 4. 위상이 어긋난 도착에서의 스케줄링 — 실측

§3의 "GPU당 6대"는 여섯 대가 같은 순간에 도착해 한 배치로 묶인다는 가정 위에 있습니다.
실제 로봇팔은 각자 자기 제어 루프를 자기 시작 시점부터 돌리므로 위상이 어긋납니다.
`bench_scheduler.py`가 그 조건을 실제 모델·실제 GPU에 재생합니다 — 로봇 6대에 각각
0/40/85/120/190/230 ms 위상을 주고 250 ms마다 반복(8사이클, 총 48요청), 시간은
시뮬레이션이 아니라 벽시계입니다. 요구 부하는 6대 ÷ 250 ms = **24 req/s**입니다.

| 정책 | 처리량 | e2e 평균 | e2e p99 | 데드라인 미스 |
| --- | ---: | ---: | ---: | ---: |
| `serial` (현재 서버) | 16.75 req/s | 474.6 ms | 886.0 ms | 75.0% |
| `batch` (동적 배칭) | **22.10 req/s** | **200.4 ms** | 320.3 ms | **18.8%** |
| `continuous` (denoise 링, 도착 즉시 admit) | 20.56 req/s | 472.3 ms | 608.5 ms | 100% |

**동적 배칭은 예상대로 크게 이깁니다.** 순차 서버는 24 req/s 요구에 16.75밖에 못 내
75%를 놓치고, 동적 배칭은 22.10까지 올라가 미스가 18.8%로 떨어집니다.

**그런데 denoise 링이 동적 배칭보다 나쁩니다.** §3의 예측(상한은 그대로, 지연 안정성만
개선)과도 어긋납니다. 틱별로 계측해 원인을 특정했습니다:

| 인플라이트 | 틱 수 | KV 재병합 | denoise 스텝 |
| ---: | ---: | ---: | ---: |
| 4 | 8 | 1.1 ms | 5.6 ms |
| 6 | 4 | 1.2 ms | 6.7 ms |
| 8 | 50 | 1.3 ms | 8.0 ms |
...
**KV 재병합은 1.1–1.3 ms로 무시할 수준이었습니다** — 설계 전 가장 우려한 비용인데
아니었습니다. 범인은 prefill입니다. 도착 간격이 42 ms인데 틱이 8 ms라 매 틱 대기열에
한 대만 있어 **로봇마다 혼자 prefill됩니다.** 48회의 B=1 prefill이 각각 32 ms씩 링을
멈춰 세웁니다. GPU 시간 총합으로 `continuous`는 2.35초(prefill 1.55 + denoise 0.80),
`batch`는 1.68초로, 링이 40% 더 많은 일을 합니다.

§3에서 "prefill은 배치를 거의 안 탄다(로봇당 1.3배)"고 관찰한 것이 여기서 되돌아옵니다.
배치 이득이 작다고 해서 **배치를 포기해도 된다는 뜻은 아니었습니다** — B=1 prefill을
48번 하는 것과 B=8을 6번 하는 것의 차이가 처리량을 뒤집습니다.

### 여유가 있을 때는 어떤가 — 로봇 4대

링이 노리는 이득(늦게 온 로봇이 다음 배치 대신 다음 denoise 스텝에 합류)은 합류할 여유가
있어야 나타납니다. 6대는 포화 구간이므로 4대로 다시 쟀습니다(요구 16 req/s, 용량 약 22):

| 정책 | 처리량 | e2e 평균 | e2e p99 | 데드라인 미스 |
| --- | ---: | ---: | ---: | ---: |
| `serial` | 15.96 req/s | **66.7 ms** | 70.8 ms | **0%** |
| `batch` | 15.81 req/s | 77.1 ms | 88.5 ms | 0% |
| `continuous` | 15.45 req/s | 215.1 ms | 265.2 ms | 12.5% |

**링은 여유 구간에서도 가장 나쁩니다.** 순차 서버의 3배가 걸립니다. 그리고 4대에서는
동적 배칭조차 필요 없습니다 — 순차 서버가 66.7 ms에 미스 0%로 처리합니다.

### 왜 링이 이 박스에서 지는가 — §2로 되돌아옵니다

링은 요청 하나를 **CUDA 그래프 1회 실행에서 11회(prefill + denoise 10회)로 쪼갭니다.**
§2에서 이 박스는 GPU-busy가 75%인 호스트 바운드라고 측정했고, 그렇다면 실행 횟수를 늘리는
것이 그대로 비용입니다. 실제로 그렇게 나옵니다:

| | §3 순수 계산 | 링에서 실측 |
| --- | ---: | ---: |
| denoise 1스텝 (n=1) | 2.2 ms | **3.0–3.5 ms** |

스텝마다 약 1 ms의 그래프 런치·스케줄링 오버헤드가 붙고, 10스텝이면 그것만 10 ms입니다.
여기에 매 틱의 KV 재병합(1.1–1.3 ms)과 다른 요청의 prefill 대기가 더해집니다.

**이것은 이 박스의 성질이지 링이라는 기법의 결함이 아닙니다.** GPU가 병목인 박스라면
같은 계산이 반대로 나올 수 있습니다. 다만 여기서는, 그리고 §2의 진단이 유효한 한,
**요청을 더 잘게 쪼개는 모든 기법이 불리합니다 — 레이어 축 chunked prefill을 포함해서.**
그쪽은 요청을 11조각이 아니라 16조각 이상으로 나누는 방향이므로, 이 결과를 보고
구현하지 않았습니다.

### 교훈

- **동적 배칭만 하세요. 그리고 그것도 포화 구간에서만 의미가 있습니다.**
  6대에서 미스 75% → 18.8%로 확실한 이득이지만, 4대에서는 순차 서버가 이미 미스 0%입니다.
- **denoise 링은 이 박스에서 하지 마세요.** 두 구간 모두에서 가장 나쁘고, admission을
  배치로 바꿔도(대기 3대 또는 25 ms) 달라지지 않았습니다 — 포화에서는 완료가 슬롯을
  하나씩만 열어 prefill이 B=1로 고정되기 때문입니다.
- **6대는 여전히 빠듯합니다.** 가장 좋은 정책도 22.10 req/s로 요구 24 req/s에 못 미쳐
  18.8%를 놓칩니다. §3의 216.5 ms는 순수 GPU 시간이고, 관측값 조립(배치당 5–7 ms)과
  스케줄러 오버헤드가 그 위에 얹힙니다.
- **§3의 "prefill은 배치를 거의 안 탄다(1.3배)"를 배치를 포기해도 된다는 뜻으로 읽으면
  안 됩니다.** B=1 prefill 48회와 B=8 6회의 차이가 처리량을 뒤집습니다.
- **스트림으로 공간을 겹치는 것도 안 됩니다.** prefill이 B=1에서 이미 SM 포화라
  밑에 깔 자리가 없고, §2의 25% 구멍은 한 스트림에 깊게 큐잉하면 이미 메워집니다.

원본: `profiling/baselines/a100/bench_scheduler.json`(6대),
`bench_scheduler_4robots.json`, `bench_scheduler_6robots_admit.json`.
로그 `logs/2026-08-04_20-30_scheduler_full3.log`, `logs/2026-08-04_21-00_ring_diag.log`,
`logs/2026-08-04_21-30_scheduler_final.log`, `logs/2026-08-04_22-00_scheduler_4robots.log`.

### 축을 바꿔서 겹치기 — CUDA 스트림 2개 (안 됩니다)

여기까지 실패한 두 방향(denoise 링, 레이어 축 chunked prefill)은 모두 **요청을 더 잘게
쪼개는** 쪽이었고, 호스트 바운드 박스에서 실행 횟수를 늘려 졌습니다. 그렇다면 조각 수를
그대로 두고 **공간으로 겹치는** 방향은 어떤가 — prefill 한 덩어리와 denoise 한 덩어리를
서로 다른 CUDA 스트림에 나란히 던지는 것입니다. 근거도 있어 보였습니다: prefill은
compute-bound, denoise는 저배치에서 점유율이 낮고, GPU-busy가 75%라 노는 구간이 있으며,
MoT라 가중치 세트도 분리(prefill=PaliGemma, denoise=expert 300m)돼 경합이 없습니다.

`bench_stream_overlap.py`로 쟀습니다:

| | 시간 |
| --- | ---: |
| prefill B=2 | 80.12 ms |
| denoise B=4 × 4스텝 | 40.62 ms |
| 순차 실행 | 108.13 ms |
| **스트림 2개** | **121.36 ms** |
| 완전 겹침 시 하한 | 80.12 ms |

**겹치지 않고, 13.2 ms 더 느립니다**(overlap efficiency −32.6%). 이유가 둘입니다.

**(1) prefill 밑에 자리가 없습니다.** §3의 수치가 이미 말하고 있었습니다 — prefill이
B=1 33.1 ms, B=2 64.6 ms로 **2배 일에 1.95배**입니다. 여유 SM이 있었다면 B=2가 2배보다
싸야 합니다. prefill은 **배치 1에서 이미 SM 포화**이고, 포화된 compute-bound 단계 밑에는
깔아넣을 공간이 없습니다. "prefill은 배치를 거의 안 탄다"는 §3의 관찰은 compute-bound의
증거가 맞지만, 그 함의는 **동시 실행의 근거가 아니라 반증**입니다.

**(2) §2의 25% 구멍은 공간이 아니라 시간이었습니다.** 요청당 12번의 그래프 런치 사이에
호스트를 기다리는 공백이고, **한 스트림에 둘 다 큐에 넣으면 이미 메워집니다.** 위 표가
직접 보여줍니다 — 따로 잰 합(80.12 + 40.62 = 120.74)보다 순차 실행(108.13)이 12.6 ms
빠릅니다. 큐가 깊어져 호스트 공백이 흡수된 것입니다. 두 번째 스트림은 채울 구멍이 없는
상태에서 스트림 관리 비용만 더합니다.

**CUDA 그래프 모드에서는 아예 실행조차 안 됩니다.** `max-autotune`으로 두 컴파일 영역을
동시에 던지면 inductor의 cudagraph trees가 정적 입력 버퍼를 공유해 서로를 덮어쓰고,
임베딩 인덱스가 vocab 범위(257152)를 벗어나 device-side assert로 죽습니다. 쓰려면
그래프를 **수동 캡처해 별도 풀·별도 스트림에 올려야** 하며 `torch.compile`에 맡길 수 없습니다.

> 단서: 위 측정은 그래프 없는 모드이고, PyTorch 할당자가 사이드 스트림에서 동기화를
> 유발했을 수 있습니다. 수동 캡처·사전 할당·MPS로 손질하면 −32.6%가 0% 근처까지는 갈
> 것입니다. 다만 **(1)의 SM 포화라는 상한은 그와 무관하게 남습니다** — 손질의 최선이
> "손해를 없애는 것"이지 이득이 아닙니다.

원본: `profiling/baselines/a100/bench_stream_overlap.json`,
로그 `logs/2026-08-04_22-45_stream_overlap.log`(그래프 모드 크래시),
`logs/2026-08-04_23-00_stream_overlap_nocg.log`.

### 이 벤치를 재현할 때 — 컴파일이 지연으로 둔갑합니다

이 표를 얻기까지 세 번의 측정이 전부 틀렸고, 매번 **에러가 아니라 말도 안 되는 지연**으로
나타났습니다(동적 배칭 2.62 req/s, 요청 하나가 240초). 원인은 순서대로:

1. 워밍업 형상을 손으로 맞추다 링의 KV를 놓침 — 단일 행 캐시를 합쳐 만든 것이라
   직접 prefill한 배치와 guard가 다릅니다.
2. dry run에만 의존 — 배치 구성이 타이밍에 따라 정해지므로 그 패스에 안 나온 크기는
   워밍이 안 됩니다.
3. 형상마다 한 번만 호출 — **inductor의 cudagraph trees는 같은 형상의 두 번째 호출에서
   그래프를 기록합니다.** 첫 호출은 eager로 흘려보냅니다. 그래서 8.3초짜리 기록이 측정
   구간에서 터졌고, 근처에 AUTOTUNE 로그가 전혀 없다는 점이 재컴파일이 아님을 말해줬습니다.
   워밍업 로그에서 B=3~8이 0.4초 만에 "끝난" 것이 진짜 단서였습니다.

지금은 형상마다 3회씩 워밍하고, B≥3의 워밍 시간이 0.4초가 아니라 **10초대로 나오는지**를
기록이 실제로 일어났다는 증거로 씁니다.

## 5. ⚠️ 측정 위생 — 이 인스턴스에서 GPU를 공유하게 되는 두 경로
...
| --- | ---: | ---: | --- |
| 지금 (스톡 서버, 순차 처리) | 2–4대 | 4–8대 | §1 예측 4는 2대, §4는 위상이 어긋난 4대에서도 미스 0% |
| + 동적 배칭 | **6대** (미스 18.8%) | **12대** | §3 계산 216.5 ms, §4 실측 22.10 req/s |
| + denoise 링 | **권장하지 않음** | — | §4 — 두 부하 구간 모두에서 최악 |

- **동적 배칭이 유일하게 확실한 개선입니다.** 다만 이득은 포화 구간에서만 나옵니다 —
  4대까지는 순차 서버가 이미 미스 0%이고, 6대에서 미스 75% → 18.8%가 됩니다.
- 6대에서도 미스가 0이 되지는 않습니다. 요구 24 req/s에 실측 22.10 req/s로 약간 모자랍니다.
  **확실히 데드라인을 지키려면 GPU당 5대**로 잡는 것이 안전합니다.
- **요청을 더 잘게 쪼개는 방향(denoise 링, 레이어 축 chunked prefill)은 이 박스에서
  역효과입니다**(§4). §2의 호스트 병목이 유효한 한 그렇습니다.
- 그 위로 더 가려면 §2의 호스트 병목(GPU-busy 75%)을 먼저 걷어내야 합니다. GPU를 바꾸는
  것보다 **런치 수를 줄이는** 쪽이 남은 여지입니다 — 링은 정확히 반대 방향이었습니다.
