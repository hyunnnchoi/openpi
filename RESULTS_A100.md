...
- **동적 배칭이 먼저이고, 이득의 대부분이 거기 있습니다.** 위상이 어긋난 현실 조건에서도
  16.75 → 22.10 req/s, 미스 75% → 18.8%입니다.
- **denoise 링은 prefill을 모아 돌리지 않으면 손해입니다.** admission을 배치로 하지 않으면
  §3에서 계산한 이론 상한 근처에도 못 갑니다.
- **6대는 이 박스에서 아직 빠듯합니다.** 가장 좋은 정책도 22.10 req/s로 요구 24 req/s에
  못 미쳐 18.8%를 놓칩니다. §3의 216.5 ms는 순수 GPU 시간이고, 여기에는 관측값 조립
  (배치당 5–7 ms)과 스케줄러 오버헤드가 더해집니다.

원본: `profiling/baselines/a100/bench_scheduler.json`,
